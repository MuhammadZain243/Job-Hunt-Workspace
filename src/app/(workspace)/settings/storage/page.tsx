...
import { getStorageSettingsFeedback } from "@/app/(workspace)/settings/feedback";
import {
...
export default async function StorageSettingsPage({
  ...
  const feedback = getStorageSettingsFeedback(params);
  ...
  return (
    <FadeIn className="space-y-5">
      <SettingsFeedbackToast
        ...
        clearPath="/settings/storage"
      />

      <div>
        <h2 className="text-xl font-semibold tracking-tight">CV storage</h2>
        <p className="text-muted-foreground mt-1 text-sm leading-6">
          Choose the active provider and connect Cloudinary, S3, or local
          development storage.
        </p>
      </div>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="border-border/80 bg-muted/25 rounded-2xl border p-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-muted-foreground text-sm leading-6">
                Provider secrets stay server-side and are encrypted in MongoDB.
                The browser only sees masked labels and connection state.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="border-border/80 rounded-2xl shadow-none">
        ...
        <CardContent className="space-y-4">
          ...
          <form action={saveCloudinaryConnectionAction} className="space-y-4">
      ...
      <Card className="border-border/80 rounded-2xl shadow-none">
        <CardHeader>
          ...
          <CardDescription>
            Connect a private bucket for CV storage with encrypted credentials.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {isS3Connected ? (
